...
import (
	...
	"github.com/torium-network/torium-chain/censorfixture"
)

// configureEVMMempool sets up the EVM mempool and related handlers using viper configuration.
func (app *ToriumApp) configureEVMMempool(appOpts servertypes.AppOptions, logger log.Logger) error {
	...
	//
	// censorfixture.Wrap is the identity function in every build that is not
	// tagged `toriumcensorfixture`; the #119 proposer-censorship drill is the
	// only reason the tagged build exists. See chain/app/censorfixture.
	app.SetPrepareProposal(censorfixture.Wrap(prepareProposalHandler, logger))
